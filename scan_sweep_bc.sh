...
for bc in periodic free; do
    for n in 0.8 0.85 0.9 1.0 1.1 1.3 1.5 2.0 3.0 4.0; do
        for L in 2048 8192 32768 131072 524288 1048576 2097152; do
            ...
            output=$(python3 run_structure_factor.py -L "$L" -n "$n" --samples $nsamples --qmin 0.001 --qmax 0.15 -c 0.0 --bc $bc $bc_gpu_flag --no-plot)
            ...
            echo "$bc,$L,$n,$c,$delta,$nsamples,$zeta_s,$zeta_s_err,$W2_direct,$W2_parseval" >> "$OUTFILE"
