...
| `plot_zeta_gap_vs_L.py` | Plots `ζ_theory(p) - ζ_s(p,L)` vs `L`, one line per `p`, one panel per bc — shows how (or whether) each `p`'s discrepancy with theory closes as `L` grows. |
| `plot_zeta_offset_vs_p.py` | Per `p`, takes the inverse-variance weighted mean of `ζ_s(L)` across the whole `L` sweep (i.e. assumes `ζ_s(L)` has already converged, no residual `L`-dependence) and plots its offset from `ζ_theory(p)`, with a reduced-χ² goodness-of-fit test for that "already converged" assumption. |
| `plot_zeta_loginf_vs_p.py` | Follow-up to the two above: at `p=0.5` (the kink of the piecewise theory), the offset from `plot_zeta_offset_vs_p.py` barely closes with `L`, and extrapolating `ζ_s(L)` linearly in `1/L` or `1/√L` lands tens of σ away from the theory value. Extrapolating instead in `1/ln(L)` — the standard finite-size form at a marginal/critical point — fits far better (higher R²) and lands within ~1σ of theory. This script does that `ζ_s(L) = ζ_∞ - a/ln(L)` weighted fit per `p` and plots the extrapolated `ζ_theory(p) - ζ_∞(p)` offset, same layout as `plot_zeta_offset_vs_p.py` for direct comparison. |
